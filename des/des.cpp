...
namespace DES
{
    ...
    namespace impl
    {
        ...
        std::array<ubyte, 8>
        processMessage(const std::array<std::bitset<48>, 17> &ks, const std::array<ubyte, 8> &message)
        {
            for (int i = 0; i < 8; i++)
            {
                if (message[i] == '0')
            ...
            for (int i = 1; i < 17; ++i)
            {
                ...
                left[i - 1] ^= fs;
                right[i] = left[i - 1];
                std::cout << std::dec << "round " << i
                          << std::hex << "  left: " << left[i] << "right: " << right[i] << std::endl;
            }
            std::cout << std::endl;
...
void run(const std::vector<DES::ubyte> &m)
{
    using namespace std;
    using namespace DES;
    key_t key = {
    ...
    auto encoded = encrypt(key, m);
    cout << "Encoded : " << encoded << endl << endl;

    auto decoded = decrypt(key, encoded);
...
//            0x01, 0x02, 0x03, 0x04, 0x00, 0x00, 0x00, 0x00};
    string message1 = "Today is a nice day!";
    string message2 = "Today is a nice day";
    vector<ubyte> v1(message1.begin(), message1.end());
    vector<ubyte> v2(message2.begin(), message2.end());
